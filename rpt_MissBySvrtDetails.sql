SELECT name,patchname,prvpatch,device_count ,severity,prodfamily,status,type,date_posted,architecture,preference,acqisition_status,bulletin,prodid,patchuid,vendor,sqnumber,lang_name,languagecode,language
...
SELECT 
  a.patchname ,a.prvpatch, count(distinct(a.device_id)) as device_count,pb.title,pb.vendor_info_page,
   (CASE to_char(a.severity) 
    ...
    paq.status ,
 a.patchtyp as type,
 pb.date_posted,pd.architecture , 
 ...
 (CASE to_char(aq.state)
   ...
   Else 'No'
   END) AS acqisition_status,a.bulletin,a.prodfamily,a.prodid,a.name,a.patchuid,a.vendor,a.sqnumber,a.language,l.lang_name,l.languagecode
...
<<INNERFILTERSTRING>> AND paq.status= 'Not Patched'  
group by a.patchname,a.prvpatch,a.severity,paq.status,a.patchtyp,pb.date_posted,pd.architecture,aq.state,preference,title,vendor_info_page
,a.bulletin,a.prodfamily,a.prodid,a.name,a.patchuid,a.vendor,a.sqnumber,a.language,l.lang_name,l.languagecode) t1 <<OUTERFILTERSTRING>>