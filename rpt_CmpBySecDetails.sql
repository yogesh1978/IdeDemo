SELECT name,patchname ,prvpatch,device_count ,type,status,prodfamily,severity,date_posted,architecture,preference ,acqisition_status
,bulletin,prodid,patchuid,vendor,sqnumber,language,lang_name,languagecode,title,vendor_info_page
FROM  (
SELECT 
...
left join (SELECT patchname,vendor,bulletin,name,
					CASE WHEN rebootpending>0 THEN 'Reboot Pending' 
						...
						FROM patchadv a LEFT JOIN DeviceConfig b on a.device_id=b.device_id GROUP BY patchname,vendor,bulletin,name )t
...
a.bulletin,a.prodfamily,a.prodid,a.patchuid,a.vendor,a.sqnumber,a.language,l.lang_name,l.languagecode )  t1  <<OUTERFILTERSTRING>>